var update_mini_scoreboard = function(){
 $.getJSON( "/data/mini_scoreboard_data.json", function(data) {
   var entries = "";
...
var update_dashboard_problems = function(){
    $.get("/data/dashboard_problems.html", function(data){
...
$("#flag_submit_button").on ("click", function () {
	...
	var encrypted_flag = encflag (team_id, $("#flag").val(), key);
	$('#submit-status').text('');
	$.post ("/flag_submit",
...
{
	$.ajax({url: window.location.origin + "/data/announcement_data.json",
			dataType: "json",
